import { ArrowLeft } from "lucide-react";
import type { Metadata } from "next";
import Link from "next/link";
import { IdeaJournal } from "../../components/IdeaJournal";

export const metadata: Metadata = {
  title: "Idea Journal — Solo Founders",
  description:
    "Track how your thinking evolves day-to-day with notes per idea. Save and revisit your startup ideation journey.",
};

export default function IdeaJournalPage() {
  return (
    <main className="flex-1 pb-24">
      <div className="mx-auto mt-4 w-full max-w-[1200px] px-4 sm:px-6 lg:px-8">
        <Link
          href="/solo-founders"
          className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-gray-500 transition hover:text-gray-950"
        >
          <ArrowLeft className="size-4" aria-hidden="true" />
          Back to tools
        </Link>
      </div>
      <div className="mx-auto max-w-[1200px] px-4 pb-12 pt-6 sm:px-6 lg:px-8 lg:pb-16 lg:pt-10">
        <IdeaJournal />
      </div>
    </main>
  );
}
